...
class AutoPlayBot:
    ...
    def _build_game_state(self, det: Dict) -> Tuple[List, List, List]:
        ...
        for p in plants_raw:
            plants.append({
                "type": "pea_shooter",
                "row": get_row(p["y"], grid_rows),
                "col": get_col(p["x"], grid_cols),
                "x": p["x"], "y": p["y"]
            })
        ...
        for z in zombies_raw:
            zombies.append({
                "type": "zombie",
                "row": get_row(z["y"], grid_rows),
                "col": get_col(z["x"], grid_cols),
                "x": z["x"], "y": z["y"]
            })
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                for reward in det.get("sunflower_reward", []):
                ...
                if action == "plant" and det.get("pea_shooter_ready"):
                    seed = det["pea_shooter_ready"][0]
    ...
    def _draw_frame(self, frame, det: Dict, fps: int):
        ...
        # Status bar
        status = f"FPS:{fps} | Sun:{len(det.get('sun', []))} | Zombie:{len(det.get('zombie', []))}"
